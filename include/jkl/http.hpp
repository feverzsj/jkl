...
template<class Stream, class Req, class Res, class ReadBuf>
class http_conn_t
{
    Stream  _s;
    ReadBuf _rb;
    ...
    auto do_read(M& m, P... p)
    {
        return make_ec_awaiter<size_t>(
            [&](auto&& h) { http::async_read(_s, _rb, m, std::move(h)); }
            p...);
    }

    template<class M, class... P>
    auto do_write(M& m, P... p)
    {
        return make_ec_awaiter<size_t>(
            [&](auto&& h) { http::async_write(_s, m, std::move(h)); }
            p...);
    }
...
public:
    ...
    using next_layer_type   = next_layer_t<Stream>;
    using lowest_layer_type = typename next_layer_type::lowest_layer_type;
    using socket_type       = typename next_layer_type::socket_type;
    ...
    template<class... A>
    explicit http_conn_t(A&&... streamArgs) : _s(std::forward<A>(streamArgs)...) {}

    //http_conn_t& operator=(socket_type&& s) { socket() = std::move(s); }

    void set_max_read_buf(_B_<size_t> n) { if(n != max_read_buf()) _rb = ReadBuf(n.count()); }
    _B_<size_t> max_read_buf() const noexcept { return _B_(_rb.max_size()); }
    ...
    // e.g.: beast::basic_stream
    auto      & next_layer()       { return get_next_layer(_s); }
    auto const& next_layer() const { return get_next_layer(_s); }
    // e.g.: asio::basic_stream_socket
    auto      & socket()       { return next_layer().socket(); }
    auto const& socket() const { return next_layer().socket(); }
    // e.g.: asio::basic_socket
    auto      & lowest_layer()       { return socket().lowest_layer(); }
    auto const& lowest_layer() const { return socket().lowest_layer(); }

    decltype(auto) stream_native_handle() { return stream().native_handle(); }
    ...
    template<class... P> auto read_req (P... p) { return do_read (req, p...); }
    template<class... P> auto read_res (P... p) { return do_read (res, p...); }
    template<class... P> auto write_req(P... p) { return do_write(req, p...); }
    template<class... P> auto write_res(P... p) { return do_write(res, p...); }

    template<class Ep, class... P>
    auto connect_endpoint(Ep&& ep, P... p)
    {
        return make_ec_awaiter(
            [&, ep = std::forward<Ep>(ep)](auto&& h){ next_layer().async_connect(ep, std::move(h)); },
            p...);
    }

    template<class... P>
    aresult_task<> connect(aurl u, P... p)
    {
        if(auto e = u.endpoint())
            co_return co_await connect_endpoint(*e, p...);

        tcp_resolver_t<executor_type> rsv(get_executor());

        JKL_CO_TRY(eps, co_await rsv.resolve(u.hostname(), u.real_port_or_protocol(), p...));

        co_return co_await connect_endpoint(eps, p...);
    }
    ...
    {
        if(auto* name = SSL_get_servername(stream_native_handle(), TLSEXT_NAMETYPE_host_name))
            return {name};
        return {};
    ...
    aresult<> set_sni_hostname(Str const& name) noexcept
    {
        ...
        if(SSL_set_tlsext_host_name(stream_native_handle(), as_cstr(name).data()))
            return no_err;
...
>
using http_plain_conn_t = http_conn_t<beast::basic_stream<asio::ip::tcp, Executor, RatePolicy>, Req, Res, ReadBuf>;
...
template<
    ...
    class Executor   = asio::executor
>
using http_ssl_conn_t = http_conn_t<beast::ssl_stream<beast::basic_stream<asio::ip::tcp, Executor, RatePolicy>>, Req, Res, ReadBuf>;
...
template<
    ...
    class Executor   = asio::executor
>
using http_var_conn_t = http_conn_t<basic_var_stream<asio::ip::tcp, Executor, RatePolicy>, Req, Res, ReadBuf>;
